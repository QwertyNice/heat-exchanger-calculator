...
def Miheev_in(self):
    if self.re < 2300:
        return 0.15 * (self.re ** 0.33) * (self.pr_domain ** 0.43) * self.solve_epsilon_t_count() * self.solve_epsilon_l_lam_count()
        ...
        for j in range(len(k_correction)):
            if k_correction[j - 1][0] <= _re < k_correction[j][0]:
                return (interpolation(k_correction[j][1], k_correction[j - 1][1],
                                  ...
                                  _re)) * self.solve_epsilon_t_count()* self.pr_domain ** 0.43

    else:
        return 0.021 * (self.re ** 0.8) * self.pr_domain ** 0.43 * self.solve_epsilon_t_count() * self.solve_epsilon_l_turbulent_count()
